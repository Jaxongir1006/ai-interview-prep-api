...
func New(
	...
	provider authoauth.GoogleProvider,
) UseCase {
	return &usecase{
...
func (uc *usecase) OperationID() string { return "google-oauth-login" }

func (uc *usecase) Execute(ctx context.Context, in *Request) (*Response, error) {
	identity, err := uc.provider.VerifyIDToken(ctx, in.IDToken)
	if err != nil {
...
func (uc *usecase) resolveAccount(
	...
	oauthAcc, err := uc.domainContainer.OAuthAccountRepo().Get(ctx, oauthaccount.Filter{
		Provider:       lo.ToPtr(oauthaccount.ProviderGoogle),
		ProviderUserID: &identity.ProviderUserID,
	})
	if err == nil {
